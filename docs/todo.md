...
### 1. Pre-flight Checks
Before autopilot starts, validate the environment:
- [ ] Verify git working directory is clean
- [ ] Check that required tools exist (test runner, linter, type checker)
- [ ] Validate task file format and structure
...
- [ ] Log test output to prove the red→green transition
- [ ] Fail the requirement if test passes before implementation (test isn't testing anything)

### 3. Codebase-Aware Task Generation
The `/tasks` command could be smarter:
- [ ] Analyze existing codebase patterns before generating tasks
- [ ] Suggest test file locations based on project conventions
- [ ] Identify architectural dependencies between requirements
- [ ] Order requirements based on dependency graph
...
Currently hardcoded in the command:
- [ ] Per-project config file (`.autopilot.json` or in `package.json`)
- [ ] Per-mode defaults (tasks: 50, tests: 30, lint: 50, entropy: 30)
- [ ] Override via command args
